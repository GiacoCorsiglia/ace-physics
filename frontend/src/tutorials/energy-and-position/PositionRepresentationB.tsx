...
const sections = sectionComponents(EnergyAndPosition, [
  (f) => (<Section first>
    <Prose>
      The position representation (or wave function) of the state <M t="\ket{\psi_B}"/> from back on page 2 is given here:
 </Prose>
    <Continue commit={f.intro4Commit}/>
</Section>
  ),
  (f) => (
    <Section commits={f.intro4Commit}>
      <TextArea
        ...
        label={
          <Prose>
            a) Very roughly (in the lower figure), sketch the probability density associated with <M t="\psi_B(x)"/>.
          </Prose>}
      />
  ...
  (f) => (
    <Section commits={f.probabilityDensityCommit}>
      <TextArea
        ...
        label={
          <Prose>
            b) In the lower figure, indicate where a particle described by <M t="\psi_B(x)"/> would be MOST likely to be found if you measured its position.
          </Prose>}
      />
      <TextArea
        field={f.leastLikelyLocation}
        label={<Prose>where it is LEAST likely to be found.
          </Prose>}
      />
...
(f)=>(
  <Section commits={f.LikelyLocationCommit}>
    <TextArea
      ...
      label={
        <Prose>
          Compare your answers with those you got for <M t="\psi_A(x)"/>,  what is different now?
        </Prose>}
    />
...
(f)=>(
  <Section commits={f.EVLocationCommit}>
    <TextArea
      ...
      label={
        <Prose>
          d) At the bottom of page 2 we asked you if the second particle (in state <M t="\ket{\psi_B}"/>) was in the SAME state as <M t="\ket{\psi_A}"/>, or if was it in a DIFFERENT state. Is your answer modified in any way at this point?
        </Prose>}
    />
