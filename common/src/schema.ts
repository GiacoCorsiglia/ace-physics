 ...
 */
type NullableSchema<T extends NonNullable<Data>, Extra> = Schema<T | null> & {
...
function schema<T extends Data, Extra extends {}>(
  {
    is,
    decode: _decode,
    default: _default, // `default` is a keyword
    validators = [],
  }: {
    is(v: unknown): v is T;
    decode(v: unknown, context: Context): Decoded<T>;
    default: () => T | null;
    validators?: (<T>(v: T) => Validated<T>)[];
  },
  extra: Extra
): Schema<T> & Extra {
  const self = {
 ...
 */
export function boolean({ default: default_ = null }: Config<boolean> = {}) {
 ...
 */
export function number({ default: default_ = null }: Config<number> = {}) {
 ...
 */
export function string({ default: default_ = "" }: Config<string> = {}) {
