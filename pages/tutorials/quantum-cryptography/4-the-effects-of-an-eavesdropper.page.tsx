  ...
  sections: [
      ...
      body: (m, { responses }) => (
        <>
          <ChooseOne
            ...
            label={
              <Prose>
                In the diagram above, Eve receives a qubit from Alice. Does she
                pass on a qubit in that same state to Bob?
              </Prose>
            }
            choices={[
        ...
        messages: {
          discussion: {
            body: (s) => (
              <>
                <Callout color="blue">
                  We havent checked your answers yet, but: <br />
    ...
    section({
      ...
      body: (m) => (
        <>
          <Prose>
            Here is a table of what Eve gets. Note that a letter "R" indicates a
            case where the outcome was “random”. (You will fill in the question
            marks below.)
          </Prose>
    ...
    section({
      ...
      body: (m, s) => (
        <>
          <Prose>
            {" "}
            Use the information in the table to determine the value of Eve's bit
            (after she measures) for qubits 2 and 4.
          </Prose>
          <LabelsLeft>
            <Dropdown
              model={m.evesBitQubit2}
              label={<Prose>Qubit 2:</Prose>}
              choices={[
                ["0", 0],
                ["1", 1],
                ["random", "R"],
      ...
      guidance: {
        ...
        messages: {
          correct: {
            body: <Guidance.Agree> Correct - good work!</Guidance.Agree>,
            onContinue: "nextSection",
          },
          incorrect: {
            body: (
              <Guidance.Disagree>
                Looks like we disagree with at least some of your answers for
                Eve's measurements. It might help to write down the state Eve
                has after she does or does not apply the H-gate.
              </Guidance.Disagree>
            ),
      ...
      body: (m, s) => (
        <>
          <Prose>
            {" "}
            Use the information in the table to determine the value of Eve's bit
            (after she measures) for qubits 5 and 9.
          </Prose>
          <LabelsLeft>
            <Dropdown
              model={m.evesBitQubit5}
              label={<Prose>Qubit 5:</Prose>}
              choices={[
                ["0", 0],
                ["1", 1],
                ["random", "R"],
      ...
      guidance: {
        ...
        messages: {
          ourAnswer: {
            body: (s) => {
              if (
                s.responses?.howOftenEveMeasuresR === 50 ||
                s.responses?.howOftenEveMeasuresR === 0.5
              ) {
      ...
      body: (m) => (
        <>
          <Prose>
            ...
            measuring (to try to reverse her former action). If she did not
            apply an H-gate before, she does nothing.
          </Prose>
          <tableWithEve.Component
            model={repeatedModel(m.tableWithEve)}
            rows={[
          ...
          dynamicAnswer: {
            body: ({ responses }) => (
              ...
              >
                {responses?.eveSendsBobQubit2?.selected !== "-z" ? (
                  <p>
                    Looks like we disagree. Remember that when Eve measures a
                    state and gets a bit (0 or 1) she collapses the state into{" "}
                    <M t="\ket{0}" /> or <M t="\ket{1}" />. Then, she either
                    does or does not reapply the Hadamard gate, according to the
                    table. Given this information, can you figure out what
                    states she sends to Bob? (For qubit 2, the table says she
                    did not reapply the Hadamard.)
                  </p>
                ) : (
                  <p>
                    Correct! When Eve measures a state and gets a bit (0 or 1)
                    she collapses the state into <M t="\ket{0}" />
                    or <M t="\ket{1}" />. Then, she either does or does not
                    reapply the Hadamard gate, according to the table. (In this
                    case, she did not reapply the Hadamard.)
                  </p>
                )}
              </Guidance.Dynamic>
            ),
    ...
    section({
      ...
      guidance: {
        ...
        messages: {
          correct: {
            body: <Guidance.Agree>Great job! </Guidance.Agree>,
            onContinue: "nextSection",
          },
          incorrect: {
            body: (
              <Guidance.Disagree>
                ...
                she collapses the state into <M t="{H\ket{0}}" />
                or
                <M t="{H\ket{1}}" />. Then, she either does or does not reapply
                the Hadamard gate, according to the table. Given this
                information, can you figure out what states she sends to Bob?
                Recall the effect of the Hadamard gate on a qubit:
                <p>
                  <center>
                    <M t="H\ket{0} = \ket{+}" /> <br />
                    <M t="H\ket{1} = \ket{-}" /> <br />
            ...
            body: (
              <>
                <Guidance.HeadsUp>
                  ...
                  Hadamard choice as Alice, she will measure the original bit
                  that Alice randomly chose, and then send it along in the same
                  basis that Alice sent. If Eve makes the wrong choice, she will
                  always send a qubit in the wrong basis. <br />
                  <br />
                  To be specific: If Alice did NOT apply a Hadamard, and neither
                  does Eve, the qubit is simply sent along unaffected by Eve’s
                  measurement. (If Eve measures a 0, she sends a{" "}
                  <M t="{H\ket{0}}" />, which is what Alice sent. The same goes
                  for 1.) <br />
                  <br />
                  But if, for example, Alice sends a <M t="{\ket{0}}" />, and
                  Eve chooses differently than Alice (and thus applies a
                  Hadamard), she will turn the state into <M t="{\ket{+}}" />.
                  Her measurement result will then be random, and she will apply
                  another Hadamard, sending along either <M t="{\ket{+}}" /> or{" "}
                  <M t="{\ket{-}}" />
                  to Bob. She sends a <em>different</em> state than Alice
                  originally sent.
                  <br />
                  <br />
                  Convince yourself that in all scenarios, if Eve chooses
                  differently than Alice, she will never send the state Alice
                  sent. (But, if she chooses the same, she will always send the
                  state Alice sent.)
                </Guidance.HeadsUp>
              </>
            ),
    ...
    section({
      ...
      body: (m) => (
        <Dropdown
          ...
          label={
            <Prose>For qubit 4, what is the result of Bob's measurement?</Prose>
          }
          choices={[
            ["0", 0],
            ["1", 1],
              ...
              >
                {responses?.bobsMeasurementQubit4?.selected !== "random" ? (
                  <p>
                    ...
                    measuring the <M t="\ket{-}" /> state. (The outcome is
                    random)
                  </p>
                ) : (
                  <p>
                    Correct! Bob’s outcome will be random because he is
    ...
    section({
      ...
      body: (m, s) => (
        <>
          <Prose>
            {" "}
            For qubits 5 and 8, what are the results of Bob's measurements?
          </Prose>
          <LabelsLeft>
            <Dropdown
      ...
      guidance: {
        ...
        messages: {
          ...
          incorrect: {
            body: (
              <Guidance.Disagree>
                It might help to write down the state Bob has after he does (or
                does not apply) the H-gate. And, recall the effects of the H
                gate:
                <center>
                  <M t="H\ket{0} = \ket{+}" /> <br />{" "}
                  <M t="H\ket{1} = \ket{-}" /> <br />
                  <M t="H\ket{+} = \ket{0}" /> <br />{" "}
                  <M t="H\ket{-} = \ket{1}" /> <br />
                  with <M t="\ket{+} = \frac{1}{\sqrt{2}}(\ket{0} + \ket{1})" />{" "}
                  and
    ...
    section({
      ...
      guidance: {
        nextMessage(r) {
          const raw =
            typeof r.fractionOfMismatchedComparedSampleBits === "object" &&
            r.fractionOfMismatchedComparedSampleBits !== null
              ? (r.fractionOfMismatchedComparedSampleBits as any).selected ??
        ...
        messages: {
          correct: {
            body: (
              <Guidance.Agree>
                {" "}
                To answer this question, we compared Bob's bits to Alice's bits
                (the bottom row to the top row). Bits 7 and 8 disagree, so our
                answer is 2/7, or 29%.{" "}
              </Guidance.Agree>
            ),
            onContinue: "nextSection",
    ...
    section({
      ...
      guidance: {
        nextMessage(r) {
          const raw =
            typeof r.fractionOfMismatchedComparedBits === "object" &&
            r.fractionOfMismatchedComparedBits !== null
              ? (r.fractionOfMismatchedComparedBits as any).selected ??
        ...
        messages: {
          correct: {
            body: <Guidance.Agree> Thats correct!</Guidance.Agree>,
            onContinue: "nextSection",
          },
          incorrect: {
            body: (
              <Guidance.Disagree>
                ...
                the <M t="{\{\ket{+}, \ket{-}\}}" /> basis, giving him a random
                result that is as likely to match as mismatch.
                <hr />
      ...
      body: (
        <Prose>
          <p>
            ...
            <br />
            If their bits are all the same, they can be confident no one
    ...
    section({
      ...
      body: (m) => (
        <>
          <Prose>
            Assume a very long message was sent, and we choose to disclose only
            2 bits to test for the presence of Eve. What is the percent chance
            that both of Bob's “test” bits will match Alice's?{" "}
            <b>(Round to the nearest percent).</b>
          </Prose>
          <LabelsLeft>
            <Decimal
      ...
      guidance: {
        nextMessage(r) {
          ...
          const raw =
            typeof r.percentOfBobsTest === "object" &&
            r.percentOfBobsTest !== null
        ...
        messages: {
          correct: {
            body: (
              <Guidance.Agree>
                We agree with your answer. We know that Bob and Alice's bits
                match 75% of the time. The chance that two bits match is
          ...
          incorrect: {
            body: (
              <Guidance.Disagree>
                We know that Bob and Alice's bits match 75% of the time. The
                chance that two bits match is therefore <M t="{(0.75)^2}" /> or
                56%. If this happens, Alice and Bob fail to notice the
                eavesdropper.
              </Guidance.Disagree>
            ),
            onContinue: "nextMessage",
      ...
      body: (m) => (
        <>
          <Prose>
            ...
            and Alice <strong>fail</strong> to notice the eavesdropper? (That
            is, what are the chance that all 100 bits Bob and Alice check
            match?)
          </Prose>
        ...
        messages: {
          explanation: {
            body: (
              <>
                <Guidance.HeadsUp>
                  We haven't checked your answer, but: Each bit has a 75% chance
                  of matching, so 100 bits have a <M t="{(0.75)^{100}}" />{" "}
                  chance of matching. This is roughly a{" "}
